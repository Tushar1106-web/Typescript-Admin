...
import { fetchTodos, fetchUserById } from "../features/apiSlice";
import { Link } from "react-router-dom";
import { fetchProducts } from "../features/ProductSlice";

const Api = () => {
  const dispatch = useDispatch();
  const product = useSelector((state: any) => state.api);
  useEffect(() => {
    // dispatch();
    dispatch(fetchTodos(45));
    // dispatch(fetchProducts)
    // dispatch(fetchUserById(3245))
    // fetchTodos('sf')
  }, [dispatch]);
  ...
  return (
    <div className="grid xl:grid-cols-4 2xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1">
      {product?.product?.products?.map((item: any) => {
        return (
            ...
            <div className=" mt-14 max-w-sm w-full p-4  ">
              <div className="card flex flex-col p-10 bg-white rounded-lg shadow-2xl">
                <Link to="#">
                  <img
                    alt=""
                    className="p-8 rounded-t-lg h-80 w-80 object-cover"
                    src={item.images[0]}
                  />
                </Link>
